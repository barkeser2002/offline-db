...
# Sürüm kontrolünü kaldırın (riskli, uyumluluk sorunları olabilir)
...
try:
    ...
    # kometautils yerine basit logger ve args simülasyonu
    class KometaArgs(dict):
        def __init__(self, repo, base_dir, options, use_nightly=False):
            super().__init__()
            self.update({opt["key"]: opt["default"] for opt in options})

    class KometaLogger:
        def __init__(self, name, log_name, log_dir, is_trace=False, log_requests=False):
            self.name = name
        ...
        def header(self, args, sub=False):
        ...
        def runtime(self):
            return str(datetime.now(UTC) - self.start_time)
    ...
    print(f"Requirements Error: Requirements are not installed: {e}")
    sys.exit(0)
...
options = [
    {
        ...
        "type": "bool",
        "default": False,
        "help": "Run with extra trace logs.",
    },
    {
        "arg": "lr",
        "key": "log-requests",
        ...
        "type": "bool",
        "default": False,
        "help": "Run with every request logged.",
    },
]
...
logger = KometaLogger(
    ...
    is_trace=args["trace"],
    log_requests=args["log-requests"],
)
...
for anime in html.fromstring(requests.get(anidb_url).content).xpath("//anime"):
    ...
    try:
        ...
        pass
    try:
        anime_dicts[anidb_id]["tvdb_epoffset"] = int(
            str(anime.xpath("@episodeoffset")[0])
        )
    except ValueError:
...
try:
    ...
    edits = {}
for anidb_id, ids in edits.items():
    anidb_id = int(anidb_id)
...
if git_available and [
    item.a_path
    for item in Repo(path=".").index.diff(None)
    ...
    repo.index.commit(
        f"Update Anime IDs - {datetime.now(UTC).strftime('%Y-%m-%d %H:%M:%S UTC')}"
    )
else:
