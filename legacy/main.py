...
# Project modules
from config import COVER_DIR, UPDATED_IDS_FILE, ADAPTERS, HTTP_TIMEOUT
import db
...
def parse_jikan_data(
    jikan_data: dict, extra_ids: Optional[Dict[str, Any]] = None
) -> dict:
...
def search_in_adapters(
    ...
    match = search_in_single_adapter(
        animecix.search_animecix, "animecix", search_terms_cix
    )
    if match:
    ...
    match = search_in_single_adapter(
        lambda term: anizle.search_anizle(term, limit=10, timeout=10),
        "anizle",
        search_terms_anizle,
    )
    if match:
    ...
    match = search_in_single_adapter(
        lambda term: tranime.search_tranime(term, limit=10),
        "tranime",
        search_terms_tranime,
    )
    if match:
...
def fetch_episodes_from_animecix(source_id: str) -> list:
    ...
    try:
        ...
        for i, ep in enumerate(episodes):
            ...
            result.append(
                {"number": ep_num, "title": ep.title, "url": ep.url, "videos": videos}
            )
        return result
    except Exception as e:
            ...
            try:
                ...
                for stream in streams:
                    ...
                    if url:
                        ...
                        videos.append(
                            {"url": url, "quality": quality, "fansub": fansub}
                        )
            except Exception as ve:
                print(f"[Anizle] Video fetch error ({ep_num}): {ve}")

            result.append(
                {"number": ep_num, "title": ep_title, "slug": ep_slug, "videos": videos}
            )
        return result
    except Exception as e:
...
def update_anime_by_mal_id(
    mal_id: int, extra_ids: Optional[Dict[str, Any]] = None, skip_videos: bool = True
) -> bool:
    """
    ...
    if episodes:
        for ep in episodes:
            ...
            ep_title = (
                ep.get("title") or ep.get("title_japanese") or f"Episode {ep_num}"
            )
            if ep_num:
        ...
        print(
            f"[{mal_id}] No episode details, creating {total_episodes} placeholders..."
        )
    ...
    for adapter_name, (
        ...
        if source_id and source_anime_id:  # source_anime_id None değilse kaydet
            db.insert_or_update_anime_source(
...
def bulk_update(
    limit: Optional[int] = None, skip_updated: bool = True, parallel: int = 10
):
    ...
    if characters:
        for char_item in characters:
            ...
            char_data = {
                ...
                "image_url": char_node.get("images", {})
                .get("jpg", {})
                .get("image_url"),
            ...
            if char_internal_id:
                db.link_anime_character(
                    anime_id, char_internal_id, char_item.get("role")
                )
...
def show_video_links(mal_id: int):
    ...
    for v in videos:
        if v["episode_number"] != current_episode:
            ...
            print("-" * 40)
        print(
            f"  [{v.get('source_name', 'unknown')}] {v.get('fansub', 'unknown')} ({v.get('quality', 'default')})"
        )
...
def main():
    ...
    parser.add_argument(
        "--bulk", action="store_true", help="Bulk update from anime_ids.json"
    )
    ...
    if args.bulk:
        bulk_update(
            limit=args.limit, skip_updated=not args.retry, parallel=args.parallel
        )
        return
