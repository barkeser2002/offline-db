...
from .animecix import _video_streams, search_animecix
from .common.utils import get_ydl_opts, get_video_resolution_mpv, extract_video_info

# Try to import other adapters if available for SearchEngine
try:
    from .turkanime import TurkAnimeAdapter
except ImportError:
    TurkAnimeAdapter = None
...
class AdapterVideo:
    ...
    def oynat(self, dakika_hatirla: bool = False):
        ...
        if not exists(mpv_path):
            ...
            if not mpv_path:
                print(
                    "MPV bulunamadı! Lütfen mpv'yi yükleyin veya bin/ klasörüne koyun."
                )
                return None
            ...
            if fmts:
                try:
                    if "height" in (fmts[0] or {}):
                        self._resolution = (
                            max(fmts, key=lambda x: x.get("height") or 0).get("height")
                            or 0
                        )
                    else:
                        t = max(
                        ...
                        self._resolution = (
                            t.get("height")
                            or (720 if (t.get("tbr") or 0) > 1500 else 480)
                        ) or 0
                except Exception:
                    self._resolution = 0
            else:
...
class AdapterBolum:
    ...
    def best_video(
        ...
        picked = (
            max(streams, key=lambda s: parse_res(s.get("label") or "0p"))
            if by_res
            else streams[0]
        )
        ...
        if vid.is_working:
            ...
            return vid
        callback(
            {"current": 1, "total": 1, "player": player_label, "status": "çalışmıyor"}
        )
        return None


# SearchEngine removed or simplified as it depends on missing modules
