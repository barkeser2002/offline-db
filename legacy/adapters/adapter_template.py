...
class TemplateAnimeAdapter:
    ...
    def __init__(self):
        ...
        self.session.headers.update({"User-Agent": self.PROVIDER_CONFIG["user_agent"]})
        self.last_request = 0
    ...
    def _make_request(
        ...
        try:
            response = self.session.request(
                method, url, timeout=self.PROVIDER_CONFIG["timeout"], **kwargs
            )
    ...
    def create_episode_object(
        self, episode_data: Dict[str, Any], anime: Anime
    ) -> Bolum:
        ...
        bolum = Bolum(
            ...
            title=episode_data.get(
                "title", f"Bölüm {episode_data.get('episode_number', 0)}"
            ),
        )
    ...
    def get_anime_details(self, anime_url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            anime_data = {
                ...
                "status": "Bilinmiyor",
                "score": None,
                "provider_data": {"anime_url": anime_url, "parsed_at": time.time()},
            }
                ...
                if title_elem and url_elem:
                    ...
                    if number_elem:
                        ...
                        number_match = re.search(
                            r"Bölüm\s*(\d+)", title_elem.text, re.IGNORECASE
                        )
                        if number_match:
                            episode_number = int(number_match.group(1))
    ...
    def get_video_urls(self, episode_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for iframe in iframes:
                ...
                if iframe_src and (
                    "video" in iframe_src.lower() or "player" in iframe_src.lower()
                ):
                    video_data = {
                        "url": iframe_src,
                        "quality": "720p",
