    ...
    for sidx in _seasons_for_title(safe_id):
        ...
        for v in data.get("videos", []):
            ...
            if name in seen:
                continue
            episodes.append(
                {"name": name, "url": ep_url, "season_num": v.get("season_num")}
            )
            seen.add(name)
    return episodes
...
class CixAnime:
    ...
    def episodes(self) -> List[CixEpisode]:
        ...
        try:
            ...
            title_id = (
                hash(self.id) % 1000000 if isinstance(self.id, str) and self.id else 0
            )
        ...
        for i, e in enumerate(eps):
            out.append(
                CixEpisode(
                    title=e.get("name") or f"Bölüm {i+1}", url=e.get("url") or ""
                )
            )
        return out
