...
def _fetch_source_links(
    source: Dict[str, Any], episode_number: int
) -> List[Dict[str, Any]]:
        ...
        if source_name == "anizle":
            ...
            if ep_details:
                ...
                for vid in raw_links:
                    ...
                    if url:
                        links.append(
                            {"url": url, "quality": "default", "fansub": vid.fansub}
                        )
            ...
            if target_ep:
                ...
                for vid in raw_links:
                    links.append(
                        {"url": vid.url, "quality": vid.quality, "fansub": vid.fansub}
                    )
...
def ensure_episode_videos(
    mal_id: int, episode_number: int, anime_db_id: int, force_refresh: bool = False
):
    ...
    if not episode:
        episode_id = db.insert_or_update_episode(
            anime_db_id, episode_number, f"Episode {episode_number}"
        )
        ...
        try:
            ...
            for u_id in users_watching:
                db.add_notification(
                    u_id,
                    "update",
                    f"New episode of {anime_title} is available: Episode {episode_number}",
                    f"/player?mal_id={mal_id}&ep={episode_number}",
                )
        except Exception as e:
    ...
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=len(sources) or 1
    ) as executor:
        future_to_source = {
            executor.submit(_fetch_source_links, source, episode_number): source
        ...
        for future in concurrent.futures.as_completed(future_to_source):
            ...
            try:
                ...
                print(
                    f"[VideoService] Error processing source {source['source_name']}: {e}"
                )
