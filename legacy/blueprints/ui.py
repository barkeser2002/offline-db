...
def fetch_season_anime(
    year: int, season: str, page: int = 1, max_retries: int = 5
) -> Dict[str, Any] | None:
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 429:
                ...
                print(
                    f"[Jikan] Rate limit! Waiting {wait_time}s... (attempt {attempt+1}/{max_retries})"
                )
...
def home():
    ...
    if "user_id" in session:
        ...
        user_recommendations = db.get_personalized_recommendations(
    ...
    recommendations = []
    try:
        top_anime_res = requests.get(
            f"{JIKAN_API_BASE}/top/anime", params={"limit": 10}, timeout=5
        )
        if top_anime_res.status_code == 200:
            top_anime = top_anime_res.json().get("data", [])

        recommendations_res = requests.get(
            f"{JIKAN_API_BASE}/recommendations/anime", params={"limit": 10}, timeout=5
        )
...
def player():
    ...
    return render_template("player.html", anime=anime, episodes=episodes, current_ep=ep)
...
def discover_page():
    ...
    return render_template(
        "discover.html", genres=genres, current_filters=current_filters, results=results
    )
