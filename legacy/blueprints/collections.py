...
def collections_list():
    ...
    if "user_id" in session:
        ...
        return render_template(
            "collections.html", collections=user_collections, is_own=True
        )
...
def collection_view(collection_id):
    ...
    return render_template(
        "collection_view.html", collection=collection, is_owner=is_owner
    )
...
def api_collections():
    ...
    if request.method == "POST":
        ...
        user_collections = db.get_user_collections(session["user_id"])
        return jsonify(
            {"success": True, "collections": db.serialize_for_json(user_collections)}
        )


@collections_bp.route(
    "/api/collections/<int:collection_id>", methods=["GET", "PUT", "DELETE"]
)
def api_collection_detail(collection_id):
    ...
    if request.method == "DELETE":
        ...
        success = db.update_collection(
            collection_id, session["user_id"], name, description, is_public
        )
        return jsonify({"success": success})
        ...
        return jsonify(
            {"success": True, "collection": db.serialize_for_json(collection)}
        )
...
@collections_bp.route(
    "/api/collections/<int:collection_id>/items/<int:mal_id>", methods=["DELETE"]
)
