...
def add_comment():
    ...
    if comment_id:
        return jsonify(
            {"success": True, "comment_id": comment_id, "username": session["username"]}
        )
...
def delete_comment(comment_id):
    ...
    return (
        jsonify(
            {"success": False, "error": "Failed to delete comment or unauthorized"}
        ),
        403,
    )
...
@social_bp.route("/api/social/global-feed", methods=["GET"])
def get_global_feed():
    limit = request.args.get("limit", 50, type=int)
    feed = db.get_global_activity(limit)

    # Format for JSON serialization
    feed = db.serialize_for_json(feed)
