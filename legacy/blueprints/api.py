...
from services.video_service import (
    ensure_episode_videos,
    check_video_link_alive,
    remove_dead_video_link,
)
...
def player():
    ...
    if not episodes and total_eps:
        episodes = [
            {"episode_number": i, "title": f"Episode {i}", "has_video": False}
            for i in range(1, int(total_eps) + 1)
        ]

    return render_template_string(
        PLAYER_TEMPLATE, anime=anime, episodes=episodes, current_ep=ep
    )
...
def proxy():
    ...
    try:
        ...
        if not parsed_url.hostname or not any(
            parsed_url.hostname.endswith(domain) for domain in ALLOWED_PROXY_DOMAINS
        ):
...
def api_json_endpoint():
    ...
    if not mal_id:
        return (
            jsonify(
                {
                    "error": "malid parametresi gerekli",
                    "usage": "/api/json?malid=21&episode=1",
                }
            ),
            400,
        )
    ...
    host = request.host_url.rstrip("/")
    cover_url = (
        f"{host}/covers/{mal_id}.jpg"
        if anime.get("cover_local")
        else anime.get("cover_url")
    )
    ...
    # Episode varsa -> Video linkleri ile birlikte
        ...
        if not videos:
            ...
            print(
                f"[API] {mal_id} ep.{episode} için video yok, adaptörlerden çekiliyor..."
            )
        ...
        def check_and_process_video(video):
            ...
            if is_alive and stream_info:
                ...
                return {"alive": False, "video_id": video_id, "url": video_url}
        ...
        if dead_videos:
            ...
            if len(working_videos) < 2:
                print(
                    f"[API] {mal_id} ep.{episode} için çalışan video az, adaptörlerden yenileniyor..."
                )
                ensure_episode_videos(mal_id, episode, anime_db_id, force_refresh=True)

                # Yeni eklenen videoları kontrol et
                new_videos = db.get_video_links(
                    anime_id=anime_db_id, episode_number=episode
                )
                for v in new_videos:
                    v_dict = cast(Dict[str, Any], v)
                    # Zaten kontrol ettiğimiz videoları atla
                    if any(
                        wv.get("original_url") == v_dict.get("video_url")
                        for wv in working_videos
                    ):
                        continue

                    is_alive, stream_info = check_video_link_alive(
                        v_dict.get("video_url", ""), timeout=8
                    )
                    if is_alive and stream_info:
...
def api_stream(mal_id, episode):
        ...
        def cleanup_dead():
            for dl in dead_links:
                if dl.get("video_id"):
                    ...
                    print(
                        f"[LinkCheck] Ölü link temizlendi: {dl.get('url', 'unknown')}"
                    )
    ...
    return jsonify({"mal_id": mal_id, "episode": episode, "videos": result_videos})
...
def api_season_anime(year, season):
    ...
    return jsonify(
        {"year": year, "season": season, "count": len(result), "anime": result}
    )
        ...
        for future in as_completed(futures):
            try:
                ...
                results["details"].append(
                    {"mal_id": mid, "status": "failed", "reason": str(e)}
                )
...
def api_sync_covers():
    ...
    if not mal_ids:
        ...
        if conn:
            ...
            if missing_only:
                cursor.execute(
                    "SELECT mal_id, cover_url FROM animes WHERE cover_local IS NULL OR cover_local = ''"
                )
            else:
                cursor.execute(
                    "SELECT mal_id, cover_url FROM animes WHERE cover_url IS NOT NULL"
                )
            rows = cursor.fetchall()
            mal_ids = [
                (
                    cast(Dict[str, Any], r)["mal_id"],
                    cast(Dict[str, Any], r)["cover_url"],
                )
                for r in rows
            ]
            cursor.close()
        ...
        try:
            ...
            if local_path:
                ...
                if conn:
                    cursor = conn.cursor()
                    cursor.execute(
                        "UPDATE animes SET cover_local = ? WHERE mal_id = ?",
                        (local_path, mal_id),
                    )
                    conn.commit()
                    cursor.close()
...
def api_sync_videos():
    ...
    results = {"success": 0, "failed": 0, "videos_found": 0, "details": []}
    ...
    def fetch_videos_for_anime(mal_id: int) -> dict:
        ...
        with ThreadPoolExecutor(
            max_workers=min(len(target_episodes), 20)
        ) as ep_executor:
            futures = [
                ep_executor.submit(fetch_episode_videos, ep) for ep in target_episodes
            ]
        ...
        return {
            "mal_id": mal_id,
            "status": "success",
            "episodes_processed": len(target_episodes),
        }
    ...
    def full_sync_anime(mal_id: int) -> dict:
        ...
        try:
            ...
            if sync_covers:
                ...
                if anime:
                    anime_dict = cast(Dict[str, Any], anime)
                    if anime_dict.get("cover_url") and not anime_dict.get(
                        "cover_local"
                    ):
                        local_path = main.download_cover(
                            anime_dict["cover_url"], mal_id
                        )
                        if local_path:
                            conn = db.get_connection()
                            if conn:
...
def api_adapters_status():
    ...
    for name, enabled in ADAPTERS.items():
        status[name] = {"enabled": enabled, "available": False}
...
def api_validate_single_link():
    ...
    result = {"url": url, "is_alive": is_alive, "stream_info": stream_info}
    ...
    if not is_alive and remove_if_dead:
        ...
        if conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE video_links SET is_active = FALSE WHERE video_url = ?", (url,)
            )
    ...
    if remove_dead and dead_link_ids:
        ...
        if conn:
            ...
            for link_id in dead_link_ids:
                cursor.execute(
                    "UPDATE video_links SET is_active = FALSE WHERE id = ?", (link_id,)
                )
            conn.commit()
...
def api_validate_batch_links():
    ...
    # Paralel kontrol
    print(
        f"[LinkValidation] {len(links)} link kontrol ediliyor ({parallel} paralel)..."
    )
    ...
    with ThreadPoolExecutor(max_workers=parallel) as executor:
        ...
        for i, future in enumerate(as_completed(futures)):
            ...
            if (i + 1) % 100 == 0:
                print(
                    f"[LinkValidation] İlerleme: {i+1}/{len(links)} (Canlı: {alive_count}, Ölü: {dead_count})"
                )
    ...
    return jsonify(
        {
            ...
            "message": f"{dead_count} ölü link bulundu"
            + (f", {deleted_count} kalıcı olarak silindi" if permanent else ""),
        }
    )
...
def fetch_episodes_from_source(mal_id: int, source_name: str):
    ...
    try:
        ...
        if conn:
            ...
            if not source_match:
                return (
                    jsonify({"error": f"Anime '{source_name}' kaynağında bulunamadı"}),
                    404,
                )

            source_anime_id, source_slug = (
                source_match["source_anime_id"],
        ...
        for ep_data in episodes_data:
            try:
                if source_name == "animecix":
                    ep_num = ep_data.get(
                        "season_num", 1
                    )  # AnimeciX'te bölüm numarası yok gibi
...
@api_bp.route(
    "/api/anime/<int:mal_id>/episode/<int:episode_number>/stream/<source_name>"
)
                    ...
                    return jsonify(
                        {
                            ...
                            "source": source_name,
                            "referral_required": source_name
                            in [
                                "anizle",
                                "turkanime",
                            ],  # Bazı kaynaklar referral header istiyor
                        }
                    )
                else:
