                            ...
                            <CardBody className="flex flex-row gap-4 p-3">
                              <div className="w-24 aspect-video bg-black/50 rounded flex-shrink-0 overflow-hidden">
                                {ep.cover_image ? (
                                  <img
                                    src={ep.cover_image}
                                    alt={`Thumbnail for Episode ${ep.number} - ${ep.title}`}
            ...
            <div className="grid grid-cols-2 gap-3">
              {anime.characters?.slice(0, 6).map((char) => (
                <div key={char.id} className="flex items-center gap-2">
                  <Avatar src={char.character.image_url} alt={char.character.name} size="md" />{" "}
...
function Avatar({ src, alt, size }: { src: string; alt?: string; size?: string }) {
  return (
    <div
      className={`rounded-full overflow-hidden bg-white/10 flex-shrink-0 ${size === "md" ? "w-10 h-10" : "w-8 h-8"}`}
    >
      {src ? <img src={src} alt={alt || "Avatar"} className="w-full h-full object-cover" /> : null}
    </div>
  );
}
