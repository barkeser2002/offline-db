...
try:
    ...
    class KometaLogger:
        ...
        def screen_width(self, width): pass
        def header(self, args, sub=False): print(f"{self.name} Started")
        def separator(self, msg=""): print("-" * 50)
        def start(self): pass
        def info(self, msg): print(f"INFO: {msg}")
        def error(self, msg): print(f"ERROR: {msg}")
        def warning(self, msg): print(f"WARNING: {msg}")
        def runtime(self): return str(datetime.now(UTC) - self.start_time)
