...
from typing import Dict, Any, cast, Optional, List, Set, Union

# Proje modülleri
from config import (
    JIKAN_API_BASE, JIKAN_RATE_LIMIT, COVER_DIR, 
...
def search_in_adapters(title: Optional[str], title_english: Optional[str] = None, title_japanese: Optional[str] = None) -> dict:
    ...
    # AnimeCiX
    if ADAPTERS.get("animecix"):
        try:
            for term in search_terms:
                cix_results = animecix.search_animecix(term)
                if cix_results:
                    best_match = max(cix_results, key=lambda x: similarity_score(term, x[1]))
                    if similarity_score(term, best_match[1]) > 0.5:
                        results["animecix"] = (best_match[0], best_match[0], best_match[1])
                        break
        except Exception as e:
            print(f"[AnimeCiX] Arama hatası: {e}")
    ...
    if ADAPTERS.get("anizle"):
        try:
            for term in search_terms:
                try:
                    anz_results = anizle.search_anizle(term, limit=10, timeout=10)
                    if anz_results:
                        best_match = max(anz_results, key=lambda x: similarity_score(term, x[1]))
                        if similarity_score(term, best_match[1]) > 0.5:
                            results["anizle"] = (None, best_match[0], best_match[1])
                            break
                except Exception:
                    break
        except Exception as e:
            print(f"[Anizle] Arama hatası: {e}")
    
    # TRAnime - hata toleranslı
    if ADAPTERS.get("tranime"):
        try:
            for term in search_terms:
                try:
                    tr_results = tranime.search_tranime(term, limit=10)
                    if tr_results:
                        best_match = max(tr_results, key=lambda x: similarity_score(term, x[1]))
                        if similarity_score(term, best_match[1]) > 0.5:
                            results["tranime"] = (None, best_match[0], best_match[1])
                            break
                except Exception:
                    break
        except Exception as e:
        ...
        try:
            for term in search_terms:
                try:
                    ...
                    if ta_results:
                        best_match = max(ta_results, key=lambda x: similarity_score(term, x[2]))
                        if similarity_score(term, best_match[2]) > 0.5:
