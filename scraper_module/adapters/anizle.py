...
def _http_post(
    ...
    try:
        ...
        try:
            ...
            session = curl_requests.Session(impersonate="chrome110")
            return session.post(
                url, headers=default_headers, timeout=timeout, data=data
            )
        except ImportError:
            pass
...
def search_anizle(
    query: str, limit: int = 20, timeout: int = 60
) -> List[Tuple[str, str]]:
    """
...
def _fetch_all_episodes_from_page(
    slug: str, timeout: int = 60
) -> List[Tuple[str, str]]:
...
def _get_episodes_remote(slug: str, timeout: int = 60) -> List[Tuple[str, str]]:
    ...
    try:
        response = requests.get(f"{SERVER_URL}/anizle/episodes/{slug}", timeout=timeout)
        response.raise_for_status()
        results = response.json()
        return results if isinstance(results, list) else []
    except Exception as e:
...
def _get_video_stream_from_player(
    player_id: str, video_name: str
    ...
    try:
        ...
        if data.get("videoSource"):
            return {"url": data["videoSource"], "label": video_name, "type": "direct"}
    ...
    try:
        ...
        for tr_url, fansub_name in matches:
            if tr_url not in seen_urls:
                ...
                translators.append({"url": tr_url, "name": fansub_name or "Fansub"})
...
def _process_single_video(video_info: Dict[str, str]) -> Optional[Dict[str, str]]:
    ...
    try:
        ...
        return _get_video_stream_from_player(
            fireplayer_id, f"{fansub_name} - {video_name}"
        )
...
def get_episode_streams(
    episode_slug: str, timeout: int = HTTP_TIMEOUT
) -> List[Dict[str, str]]:
    """
    ...
    for translator in translators:
        ...
        for video in videos:
            all_videos.append(
                {"url": video["url"], "name": video["name"], "fansub": fansub_name}
            )
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {
            executor.submit(_process_single_video, video): video for video in all_videos
        }
...
def _get_streams_remote(
    episode_slug: str, timeout: int = HTTP_TIMEOUT
) -> List[Dict[str, str]]:
    """Uzak sunucu üzerinden stream'leri al (fallback)."""
    try:
        response = requests.get(
            f"{SERVER_URL}/anizle/streams/{episode_slug}", timeout=timeout
        )
        response.raise_for_status()
        results = response.json()
        return results if isinstance(results, list) else []
