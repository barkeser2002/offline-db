...
def search_tranime(query: str, limit: int = 10) -> List[Tuple[str, str]]:
    ...
    Önce doğrudan arama endpoint'ini dener, başarısız olursa harflerle arar.
    """
    # Önce site arama endpoint'ini dene
    try:
        session = _get_session()
        
        # Site ana sayfasından arama yap
        ...
        if resp.status_code == 200 and 'Bot Kontrol' not in resp.text:
            results = []
            # Arama sonuçlarından anime linklerini çek
        ...
        print(f"[TRAnime] Doğrudan arama hatası: {e}")
    
    # Fallback: Harflerle ara
