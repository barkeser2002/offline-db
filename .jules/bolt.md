...
## 2026-03-10 - Redundant ORM prefetching in API endpoints
**Learning:** Adding `select_related` or `prefetch_related` in DRF Views does not automatically mean performance improves. If the associated nested fields are never output by the Serializer (e.g. Serializer only outputs primary keys), these database JOINs only add hidden computational overhead and slower response times without any benefit.
**Action:** When reviewing querysets for DRF ViewSets, strictly ensure that `select_related` and `prefetch_related` exactly match only the fields required by the `Serializer` class.
