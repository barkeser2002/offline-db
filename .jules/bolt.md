...
## 2025-03-09 - AnimeViewSet List Action N+1 Query Optimization
**Learning:** The `/api/v1/anime/` list endpoint was making an N+1 query. `AnimeListSerializer` accesses the `genres` many-to-many field via `GenreSerializer(many=True)`, but `AnimeViewSet.get_queryset()` was only applying `.prefetch_related('genres')` conditionally when `self.action == 'retrieve'`. This caused it to issue a separate `SELECT` query to fetch genres for every single anime returned in the list response.
**Action:** Always return `queryset.prefetch_related('genres')` for the `list` action as well to ensure that the many-to-many relationship is bulk-fetched in a single query, significantly dropping the query count.
