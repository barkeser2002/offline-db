...
## 2025-03-10 - Badge System distinct().count() Optimization
**Learning:** `distinct().count()` skips Django's queryset caching and executes a COUNT query directly against the database on every call, causing performance degradation when used repeatedly in badge evaluations (e.g. `WatchTimeBadgeStrategy` and `ConsumptionBadgeStrategy`).
**Action:** Replaced `.values('episode').distinct().count()` with `len(set(values_list('episode_id', flat=True)))`. This offloads distinct counting to Python, reducing database round-trips and redundant COUNT queries for evaluating thresholds like weekend warrior or binge watching.

## 2025-03-10 - RoomViewSet select_related Optimization
**Learning:** `RoomViewSet` in `apps/watchparty/views.py` serializes `episode` using `EpisodeSerializer`. Because `EpisodeSerializer` does not serialize `anime` by default, standard usage doesn't cause N+1. However, if any field inside `RoomSerializer` or `EpisodeSerializer` attempts to access `season` or `anime` data, Django issues new individual database queries for every room. In testing query performance for `RoomViewSet` list APIs, missing `episode__season__anime` from `select_related` resulted in ~2 extra queries per list operation simply to fetch related seasons and animes for the episodes in the rooms.
**Action:** Added `episode__season__anime` to `select_related` in `RoomViewSet.queryset` and `RoomViewSet.my_rooms()`. This eagerly loads the necessary nested data in a single SQL JOIN instead of individual queries, drastically improving response times and database load per Watch Party API request.
